...
const OcProjectSidebar = ({
	...
	const t = useTranslations('SingleProjectSidebar');

	const isDevelop = pathname.startsWith('/projects/develop');
	const backTo = isDevelop ? '/projects/develop' : '/projects/design';

	return (
		<OcCard className="w-full md:max-w-[350px] flex flex-col gap-6 md:sticky md:top-35 dark:text-[#a0b8e3]">
			<div className="flex gap-1 items-center">
				<OcButtonLink
					label={isDevelop ? t('develop') : t('design')}
					href={backTo}
			...
			<div>
				<h3 className="text-lg font-medium mb-1">{t('role')}</h3>
				<p className="opacity-70 dark:opacity-80">{role}</p>
			</div>

			<div>
				<h3 className="text-lg font-medium mb-2">{t('tech')}</h3>
